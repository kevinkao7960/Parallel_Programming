...
*********************************************************************/
__global__ void update(int nsteps, int tpoints, float *values_d)
{
    int i;
    int j = 1 + threadIdx.x;
...
int main(int argc, char *argv[])
{
    ...
    // init device array values_d
    cudaMalloc(&values_d, (MAXPOINTS+2)*sizeof(float) );
    init_line();
    // copy values to values_d
    cudaMemcpy( values_d, values, size, cudaMemcpyHostToDevice);

    printf("Updating all points for all time steps...\n");

    // init cuda block and thread
    /*********************************************************************
    *1 Block size: 32
    *********************************************************************/
    int blockNum;
    ...
    // copy the result from device before printfinal()
